...
class TranscriptSummary:
    def __init__(self, doc_id, model="gpt-3.5-turbo"):
        self.doc_id = doc_id
        self.document = self.load_json_file_and_extract_text()
        self.chatgpt = OpenAI(temperature=0, model=model)
        self.service_context = ServiceContext.from_defaults(llm=self.chatgpt, chunk_size=1024)
        ...
        self.doc_summary_index = DocumentSummaryIndex.from_documents(self.document,
            service_context=self.service_context, response_synthesizer=self.response_synthesizer, show_progress=True)
